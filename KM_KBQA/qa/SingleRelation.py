...
from ..linking.NewLinking import RuleLinker
...
def update_entity_embeddings(encode, entity_labels, driver):
    all_entities = []
...
class QAHelper():
    ...
    def __init__(self, driver, linker):
        ...
        # all_ent_emb = update_entity_embeddings(
        #     HITBert.encode, ['Instance', 'SubGenre', 'Genre'], driver)
        # self.load_dictionary(all_ent_emb)
    ...
    def link_entity(self, sent):
        ...
        if mention_list == []:
            return []
        if len(self.cut) == 0:
            self.cut = LTP.customed_jieba_cut(sent, os.path.join(
                config.LEX_PATH, 'stopwords.txt'), cut_stop=True)
        # print('cut:', self.cut)
        res = []
        for item in mention_list:
            ...
            for key in self.all_entities:
                ...
                if filtered_key == '':
                    continue
                score = self.calculate_word_similarity(
                    self.hit_encode, converted_item, filtered_key)
    ...
    def get_child_in_father(self, sg_name, c_label, f_label):
        instances = self.driver.get_genres_by_relation(
            f_label, c_label, sg_name, reverse=True).result()
    ...
    def rel_extraction(self, neoId, cand_name, ent_name, limits, cut_words, ent_label, ent_score, thres=config.prop_ths):
        ...
        for prop in props_set:
            for word in rest_words:
                score = self.calculate_word_similarity(
                    self.hit_encode, word, normalize_prop(prop))
                ...
                if score > thres and (word, prop) not in used_pairs:
                    ...
                    res.append([neoId, cand_name, ent_name, {
                               prop: props_list[prop]}, accepted_limit, score, ent_score])
        if len(res) == 0:
            return None
        logger.info('rel_res: '+str(res))
    ...
    def qa(self, sent):
        ...
        # initialization
        self.cut = LTP.customed_jieba_cut(sent, cut_stop=True)
        # 替换规则
        ...
        sent_is_list = check_list_questions(sent, self.linker.link)
        logger.info('cut words: '+str(self.cut))
        ...
        logger.info('is_list: '+str(sent_is_list))
        all_link_res = []
        ...
        logger.info('all链接结果: '+str(all_link_res))
        ...
        for cand_name, ent_id, ent_score, ent_label, ent_neoId in all_link_res:
            if ent_label == 'SubGenre' or ent_label == 'Genre':
                c_instances, c_neoIds = self.get_child_in_father(
                    ent_id, 'Instance', ent_label)
                # 该父节点无子节点
                ...
                for i, instance in enumerate(c_instances):
                    if sent_is_list:
                        ret_list = self.list_extraction(
                            c_neoIds[i], cand_name, instance, limits, 'Instance', ent_score)
                ...
                if sent_is_list:
                    ret_list = self.list_extraction(
                        ent_neoId, cand_name, ent_id, limits, ent_label, ent_score)
                    if ret_list is not None and len(ret_list) > 0:
                        all_result.append(ret_list)
                else:
                    ret_rel = self.rel_extraction(
                        ent_neoId, cand_name, ent_id, limits, self.cut, ent_label, ent_score)
                    if ret_rel is not None and len(ret_rel) > 0:
                        all_result += ret_rel
                    # 若实体识别分之很高，那么就转而进行列举类，返回常见属性
                    elif ent_score > 0.9:
        ...
        if len(all_result) == 0 and (
            ...
            if len(all_link_res) > 0:
                ...
                label_id = self.driver.get_labels_by_name(
                    config.airport.name).result()[0]
                ent_id, ent_score, ent_label, neoId = config.airport.name, 1.0, label_id[
                    0][0], label_id[1]
            # 判断该节点是否为根节点
            if ent_label == 'Instance':
                ...
                instances, neoIds = self.get_child_in_father(
                    ent_id, 'Instance', ent_label)
                if instances is None or len(instances) == 0:
                    ret_list = self.list_extraction(
                        neoId, ent_id, ent_id, limits, ent_label, ent_score)
                    if ret_list is not None and len(ret_list) > 0:
                        all_result.append(ret_list)
                else:
                    for i, instance in enumerate(instances):
                        ret_list = self.list_extraction(
                            neoIds[i], ent_id, instance, limits, 'Instance', ent_score)
                        if ret_list is not None and len(ret_list) > 0:
                            all_result.append(ret_list)
        ...
        if not sent_is_list:
            ...
            satisfied_result = list(filter(
                lambda x: x[0][-4:] != '限制错误' and x[0][-5:] != '不满足题意' and x[-1] >= config.exact_ent_ths, all_result))
            unsatisfied_result = list(filter(
                lambda x: x[0][-4:] == '限制错误' or x[0][-5:] == '不满足题意' or x[-1] < config.exact_ent_ths, all_result))
            # 再根据关系匹配值排序
            satisfied_result = sorted(
                satisfied_result, key=lambda x: x[-2], reverse=True)
            unsatisfied_result = sorted(
                unsatisfied_result, key=lambda x: x[-2], reverse=True)
            all_result = satisfied_result + unsatisfied_result
        else:
            # 针对列举类问题，首先列举满足题意的项目
            satisfied_res = list(filter(
                lambda x: x[0][-5:] != '不满足题意' and x[0][-4:] != '限制错误' and x[-1] >= config.exact_ent_ths, all_result))
            unsatisfied_res = list(filter(
                lambda x: x[0][-5:] == '不满足题意' or x[0][-4:] == '限制错误' or x[-1] < config.exact_ent_ths, all_result))
        ...
        filtered_all_result = []
        for r in all_result:
            ent_id, ent_rel = r[1], list(r[4].keys())[
                0] if r[4] is not None else r[4]
    ...
    def calculate_word_similarity(self, encode, w1, w2, type='bert'):
        ...
        elif (w2, w1) in self.cache:
            bert_score = self.cache[(w2, w1)]
        else:
            if w1 in self.entity_cache:
                e1 = self.entity_cache[w1]
            else:
                e1 = encode(w1)
                self.entity_cache[w1] = e1
            if w2 in self.entity_cache:
                e2 = self.entity_cache[w2]
            else:
                e2 = encode(w2)
                self.entity_cache[w2] = e2
            # a1, a2 = torch.tensor(e1), torch.tensor(e2)
            # dist = torch.sum((a1 - a2) ** 2).item()
            bert_score = torch.cosine_similarity(e1, e2, dim=0).item()
    ...
    def add_node(self, neoId, ans_type, name=''):
        ...
        if neoId < 0:
            ...
            entity = Node(neoId, ent_name, self.ret_id,
                          ent_prop, ent_type, ans_type)
        self.ret_id += 1
        return entity
    ...
    def fetch_surrounding_entities(self, route, num=5):
        ...
        for i, ent in enumerate(ent_route[:-1]):
            ...
            cand_surround = self.driver.get_relations_by_id(
                ent.neoId, ent.type, tgt_type).result()
            if cand_surround is not None and len(cand_surround) > 0 and len(cand_surround[0]) == 3:
                for j, cand in enumerate(cand_surround):
                    ...
                    # 计算route中子节点与
                    score = self.calculate_word_similarity(
                        self.hit_encode, c_ent_name, ent_route[i + 1].name)
                    cand_surround[j].append(score)
            cand_surround = sorted(
                cand_surround, key=lambda x: x[3], reverse=True)
            ...
            for rel, c_ent_name, c_ent_neoId, score in cand_surround[:num]:
                ...
                s_rel = self.add_edge(
                    ent.id, s_entity.id, rel, 'surrounding_relation')
    ...
    def fetch_route_entities(self, ent):
        ...
        if ent.type == 'Instance':
            relation, f_name, f_neoId = self.driver.get_relations_by_id(
                ent.neoId, '', '').result()[0]
            linked_ent = self.add_node(f_neoId, 'route_entity')
            linked_edge = self.add_edge(
                linked_ent.id, ent.id, relation, 'route_relation')
            route.append(linked_edge)
            route.append(linked_ent)
            ent = linked_ent
        if ent.type == 'SubGenre':
            relation, f_name, f_neoId = self.driver.get_relations_by_id(
                ent.neoId, ent.type, 'Genre').result()[0]
            linked_ent = self.add_node(f_neoId, 'route_entity')
            linked_edge = self.add_edge(
                linked_ent.id, ent.id, relation, 'route_relation')
            route.append(linked_edge)
        ...
        if ent.type == 'Genre':
            relation, f_name, f_neoId = self.driver.get_relations_by_id(
                ent.neoId, ent.type, 'Center').result()[0]
            linked_ent = self.add_node(f_neoId, 'route_entity')
            linked_edge = self.add_edge(
                linked_ent.id, ent.id, relation, 'route_relation')
            route.append(linked_edge)
        ...
        for r in qa_res[:ans_num]:
            ...
            if rel is not None:
                for rel_name in rel.keys():
                    ans_prop_val = self.add_node(-1,
                                                 'answer_property_value', rel[rel_name])
                    ans_prop_rel = self.add_edge(
                        ans_ent.id, ans_prop_val.id, rel_name, 'answer_property')
            ...
            if rstct is not None:
                for rst_name in rstct.keys():
                    ans_rstct_val = self.add_node(-1,
                                                  'answer_restriction_value', rstct[rst_name])
                    ans_rstct_rel = self.add_edge(
                        ans_ent.id, ans_rstct_val.id, rel, 'answer_restriction')
                    nodes.append(
            ...
            for item in route + surround:
                ...
                if isinstance(item, Edge):
                    edges.append({'source': item.src, 'target': item.tgt,
                                  'name': item.name, 'is_ans': item.ans_type})

            nodes = sorted(nodes, key=lambda x: x['id'], reverse=False)
            # pdb.set_trace()
            ans = self.answer_generation(res_type, ans.split(
                ',')[0], cand_name, name, rel, ans_ent)
            ret['answers'].append(
                {'answer': ans, 'nodes': nodes, 'edges': edges})
    ...
    def answer_generation(self, res_type, ans, cand_name, ent_name, rel, ent_node):
        ...
        if res_type == 'list' or rel is None or len(rel) == 0:
            ...
            # 加话术
            ans = '您好，机场内有%s，位置在%s，营业时间是%s，价格为%s，电话是%s' % (
                ans_name, loc_prop_value, time_prop_value, price_prop_value, tel_prop_value)
            ans = ans + airline_ans
